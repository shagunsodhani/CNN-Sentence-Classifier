...
def read_glove_vectors(glove_vector_path):
    ...
    embeddings_index = {}
    with open(glove_vector_path) as f:
        for line in f:
...
def read_input_data(input_data_path):
    ...
    with open(os.path.join(input_data_path, "input.txt"), 'rb') as sentence_f:
        for line in sentence_f:
            sentence = str(line.strip())
            texts.append(sentence)

    with open(os.path.join(input_data_path, "label.txt"), 'rb') as class_f:
        largest_label_id = 0
        for line in class_f:
